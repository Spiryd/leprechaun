...
impl std::fmt::Display for DataManagerError {
    fn fmt(&self, f: &mut std::fmt::Formatter) -> std::fmt::Result {
        write!(f, "An error occurred in the data manager")
    }
    
}

impl Error for DataManagerError {}
...
impl DataManager {
    ...
    pub async fn get_eod_data(&self, ticker: &str, start_date: Option<NaiveDate>, end_date: Option<NaiveDate>) -> Result<Vec<Eod>, Box<dyn std::error::Error>>{
        let exists: (i64, ) = sqlx::query_as("SELECT COUNT(1) FROM eod_cache_registery WHERE ticker = $1").bind(ticker).fetch_one(&self.pool).await?;
        if exists.0 == 0 {
        ...
        let cahce_registery_entery = sqlx::query_as::<_, EodCacheRegistery>("SELECT * FROM eod_cache_registery WHERE ticker = $1")
            .bind(ticker)
            .fetch_one(&self.pool).await;
        match cahce_registery_entery {
            Ok(cahce_registery_entery) => {
                match start_date {
                    Some(start_date) => {
                        match end_date {
                            Some(end_date) => {
                                if start_date < cahce_registery_entery.start_date || end_date > cahce_registery_entery.end_date {
                                    let ohlcv = self.api.get_eod("IBM").await?;
                                    for record in &ohlcv {
                                        if cahce_registery_entery.end_date < record.date_stamp  || cahce_registery_entery.start_date > record.date_stamp {
                                            sqlx::query("INSERT INTO eod (ticker, date_stamp, open, high, low, close, volume) VALUES ($1, $2, $3, $4, $5, $6, $7)")
                                                .bind(ticker)
                                                .bind(record.date_stamp)
                                                .bind(record.open)
                            ...
                            None => {
                                if start_date < cahce_registery_entery.start_date {
                                    ...
                                    sqlx::query("UPDATE eod_cache_registery SET start_date = $1 WHERE ticker = $2")
                                        .bind(ohlcv.first().unwrap().date_stamp)
                                        .bind(ticker)
                                        .execute(&self.pool).await?;
                                    let mut eod = Vec::new();
                                    ...
                                    let eod = sqlx::query_as::<_, Eod>("SELECT * FROM eod WHERE ticker = $1 AND date_stamp >= $2")
                                        .bind(ticker)
                                        .bind(start_date)
                                        .fetch_all(&self.pool).await?;
                                    return Ok(eod)
                        ...
                        match end_date {
                            Some(end_date) => {
                                if end_date > cahce_registery_entery.end_date {
                                    ...
                                    sqlx::query("UPDATE eod_cache_registery SET end_date = $1 WHERE ticker = $2")
                                        .bind(ohlcv.last().unwrap().date_stamp)
                                        .bind(ticker)
                                        .execute(&self.pool).await?;
                                    let mut eod = Vec::new();
        ...
        Err(DataManagerError::UnknownError.into())
    }
}
